        ...
        <div style="width: 100%; display: flex; justify-content: center; margin-bottom: 20px;">
            ...
            <div v-if="data.user.role === 'EMP'">
                ...
                <el-form-item label="个人介绍">
                    <el-input :rows="4" type="textarea" v-model="data.form.description" autocomplete="off" placeholder="请输入个人介绍" />
                </el-form-item>
            </div>

            <div style="text-align: center;">
                <el-button type="primary" @click="updateUser" style="padding: 20px 30px; ">更新个人信息</el-button>
            </div>
...
if (data.user.role === 'EMP') {
  ...
  data.form = data.user // 管理员：管理员只有一些基本的数据
}
  ...
  if(data.user.role === 'EMP') {
    request.put('/employee/update', data.form).then(res => {
      if(res.code === '200') {
        ...
        userStore.updateUser(data.form) // 使用Pinia更新用户信息
        emit('updateUser')
      } else {
